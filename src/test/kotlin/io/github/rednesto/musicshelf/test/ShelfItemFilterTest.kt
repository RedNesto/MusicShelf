...
class ShelfItemFilterTest {
    ...
    val itemsToTest = listOf(
            createItem("item0"),
            createItem("items1", "key2" to "value 1"),
            createItem("item2", "key1" to "value 1", "key2" to "value3"),
            createItem("item 3", "key1" to "value 1", "key 2" to "value: 2"),
            createItem("item4", "key1" to "value2", "key2" to "value3")
    )
    ...
    @Test
    fun mixed() {
        assertEquals(0, filter(setOf("item0"), "key1" to null).size)
        assertEqualsUnordered(getItems(2, 4), filter(setOf("item"), "key2" to "value3"))
        assertEqualsUnordered(getItems(1, 2, 4), filter(setOf("it", "ms"), "key2" to null))
    }

    private fun filterKeywords(vararg keywords: String): Collection<ShelfItem> =
            ShelfItemFilter(ShelfItemFilterData(keywords.toSet(), emptyMap())).filter(itemsToTest)

    private fun filterInfo(vararg info: Pair<String, String?>): Collection<ShelfItem> =
            ShelfItemFilter(ShelfItemFilterData(emptySet(), info.toMap())).filter(itemsToTest)

    private fun filter(keywords: Set<String>, vararg info: Pair<String, String?>): Collection<ShelfItem> =
            ShelfItemFilter(ShelfItemFilterData(keywords, info.toMap())).filter(itemsToTest)

    private fun createItem(name: String, vararg info: Pair<String, String>): ShelfItem {
        val infoMap = info.toMap(mutableMapOf(ShelfItemInfoKeys.NAME to name))
        return ShelfItem(UUID.randomUUID(), Paths.get(""), infoMap, mutableListOf())
    }
